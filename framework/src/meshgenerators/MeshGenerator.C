...
MeshGenerator::getCSG(const std::string & param_name, const bool allow_invalid )
{
  const MeshGeneratorName * name = getMeshGeneratorNameFromParam(param_name, allow_invalid);
  if (!name)
    return _null_CSG;
  return getCSGByName(*name);
}
*/

/** 
std::unique_ptr<pugi::xml_document> &
MeshGenerator::getCSGByName(const MeshGeneratorName & mesh_generator_name)
{
...
MeshGenerator::generateInternal()
{
  ...
  if(meshCSG)
    meshCSG->save_file("geometry.xml");
  return mesh;
}
...
{
  std::unique_ptr<pugi::xml_document> blank{};
  return blank;
}

template <typename T>
void 
MeshGenerator::set_attribute(const std::string & name, const T & value, pugi::xml_node & node)
{
...
void 
MeshGenerator::make_surface(int surfID, const std::string & coeffs, const std::string & type, pugi::xml_node & node)
{
    node.append_child("0");
    pugi::xml_node surf = node.child("0");
    surf.set_name("surface");
    set_attribute("coeffs", coeffs.c_str(), surf);
    set_attribute("id", surfID, surf);
    set_attribute("type", type.c_str(), surf);
}

void 
MeshGenerator::make_surface(int surfID, const std::string & coeffs, const std::string & type, const std::string & boundary,pugi::xml_node & node)
{
    node.append_child("0");
    pugi::xml_node surf = node.child("0");
    surf.set_name("surface");
    ...
    set_attribute("id", surfID, surf);
    set_attribute("type", type.c_str(), surf);
    set_attribute("boundary", boundary.c_str(), surf);
}
void 
MeshGenerator::make_cell(int cellID, const std::string & material, const std::string & region, int universe, pugi::xml_node & node)
{
    node.append_child("0");
    pugi::xml_node cell = node.child("0");
    cell.set_name("cell");
    set_attribute("id", cellID, cell);
    set_attribute("material", material.c_str(), cell);
    set_attribute("region", region.c_str(), cell);
    set_attribute("universe", universe, cell);
}
void
